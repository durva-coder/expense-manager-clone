...
module.exports = {
    ...
    verify: async function(req, res){
        ...
        User.find({_id:_id}).then(async function(docs){
            ...
            if(req.body.otp==docs[0].otp){
                ...
                return res.view('user/verifyMail', {err: 'OTP is Incorrect'})
    ...
    updatePro: function(req, res){
        ...
        // finding user and updating profile details
        User.find({_id}, async function(err, user){
            var updateUser = await User.update({ _id : _id})
            .set({
                name: name,
    ...
    email: function(req, res){
        res.view('user/email', {err : null})
    },

    // getting email and password & updating a password
    getEmail: function(req, res){
        let email = req.body.email;
        console.log('sdsf',email);
        let password = req.body.password;
        console.log(password);

        // finding user from email & check whether user exists
        User.findOne({email}).then(async function(user){
            console.log('hfh',user);
            ...
            // if(user != undefined){
                ...
                // return res.redirect('/resetPassword')
                 ...
                 // if not exists then user not exists error message
                console.log('hgfhfg');
                return res.status(200).view('user/email', {err: 'User Not Exists'})
    ...
    //     res.view('user/resetPassword', {err: null})
    // },
    ...
    //             console.log('ghjgf',user);
                
    //         })

    //     }else{
    //         return res.view('user/resetPassword', {err: 'Password not Match'})
    //     }
    // },
